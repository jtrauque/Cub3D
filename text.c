...
/*   Updated: 2021/01/14 16:35:32 by jtrauque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_text_n(t_params *params, char *elements, t_pars *pars)
{
	...
	if (elements[i] == 'O' && elements[i + 1] == ' ')
	{
		...
		pars->text_n.img = mlx_xpm_file_to_image(params->mlx_ptr,
				relative_path, &pars->text_n.img_width, &pars->text_n.img_height);
		pars->text_n.data = (unsigned int *)mlx_get_data_addr(pars->text_n.img,
				&pars->text_n.bpp, &pars->text_n.size_line, &pars->text_n.endian);
	}
	else
...
void	ft_text_s(t_params *params, char *elements, t_pars *pars)
{
	...
	if (elements[i] == 'O' && elements[i + 1] == ' ')
	{
		...
		pars->text_s.img = mlx_xpm_file_to_image(params->mlx_ptr,
				relative_path, &pars->text_s.img_width, &pars->text_s.img_height);
		pars->text_s.data = (unsigned int *)mlx_get_data_addr(pars->text_s.img,
				&pars->text_s.bpp, &pars->text_s.size_line, &pars->text_s.endian);
	}
	else if (elements[i] == ' ')
	{
		i += 1;
		...
		pars->text_sprite.img = mlx_xpm_file_to_image(params->mlx_ptr,
				relative_path, &pars->text_sprite.img_width, &pars->text_sprite.img_height);
		pars->text_sprite.data = (unsigned int *)mlx_get_data_addr(
				pars->text_sprite.img, &pars->text_sprite.bpp, &pars->text_sprite.size_line,
